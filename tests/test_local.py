...
def setup_environment(openai_key: str = None, use_mock: bool = False):
    ...
    if use_mock:
        ...
        print(
            "⚠️  Using mock API key - AI requests will fail but structure will be tested"
        )
    elif openai_key:
        ...
        if not existing_key:
            print(
                "❌ No OpenAI API key provided. Use --openai-key or set OPENAI_API_KEY environment variable"
            )
            return False
    ...
    os.environ["MARKDOWN_OUTPUT_PATH"] = os.getenv(
        "MARKDOWN_OUTPUT_PATH", "./test-output"
    )
    os.environ["TEMPLATE_PATH"] = os.getenv(
        "TEMPLATE_PATH", "./templates/product-api.template.md"
    )

    # Process configuration
    os.environ["MAX_ITERATIONS"] = os.getenv(
        "MAX_ITERATIONS", "3"
    )  # Reduced for testing
    os.environ["COMPLETENESS_THRESHOLD"] = os.getenv(
        "COMPLETENESS_THRESHOLD", "80"
    )  # Lower threshold for testing
    os.environ["TIMEOUT_MINUTES"] = os.getenv(
        "TIMEOUT_MINUTES", "10"
...
def validate_setup():
    ...
    yaml_files = list(Path(yaml_input).glob("*.yaml")) + list(
        Path(yaml_input).glob("*.yml")
    )
    if not yaml_files:
...
def main():
    parser = argparse.ArgumentParser(
        description="Test YAML to Markdown Documentation Generator locally"
    )
    parser.add_argument("--openai-key", help="OpenAI API key for testing")
    parser.add_argument(
        "--mock",
        action="store_true",
        help="Use mock API key (will fail AI requests but test structure)",
    )
    parser.add_argument(
        "--input-path", help="Path to YAML input directory", default="./samples"
    )
    parser.add_argument(
        "--output-path", help="Path to output directory", default="./test-output"
    )
    parser.add_argument(
        "--template",
        help="Path to template file",
        default="./templates/product-api.template.md",
    )
    ...
    if args.template != "./templates/product-api.template.md":
