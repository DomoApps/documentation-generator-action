...
- `modelId`: The ID of the model used for summarization.
...
**Method:** `POST`  
**Endpoint:** `https://your-api-hostname.com/api/ai/v1/text/sql`
...
const config = {
  method: 'post',
  url: 'https://your-api-hostname.com/api/ai/v1/text/sql',
  headers: { 
    'Content-Type': 'application/json'
  },
  data : data
...
```bash
curl -X POST https://your-api-hostname.com/api/ai/v1/text/sql \
-H 'Content-Type: application/json' \
-d '{
  "input": "What are my total sales by region?",
  "dataSourceSchemas": [
...
- `prompt`: The SQL query prompt generated based on the input and data sources.
- `output`: The generated SQL query to fulfill the provided input.
- `modelId`: The ID of the model used to produce the SQL query.
...
```javascript
// Generate a realistic fetch example using the request body example
// Use the actual endpoint path with any path parameters filled in
// Include proper headers (Content-Type: application/json if body exists)
...
- `prompt`: The original input text provided.
- `output`: The generated text output from the model.
- `modelId`: The ID of the model used for the text generation.
...
| Parameter | Type   | Required | Description |
|-----------|--------|----------|-------------|
| `template` | string | No       | The string template with parameter placeholders for injection. |
...
```javascript
fetch('https://example.com/api/ai/v1/messages/chat', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
...
- `modelId`: The identifier of the used AI model.
- `isCustomerModel`: Indicates if the AI model is a customer-specific model.
- `sessionId`: The unique identifier for the AI session.
- `requestId`: The unique identifier for the request.
- `stopReason`: The reason why the generation was stopped.
...
- `prompt`: The prompt used for the image analysis.
- `output`: The textual description or observations derived from the image.
...
- `embeddings`: An array containing the generated text embeddings.
...
The request body for generating image embeddings.

| Parameter            | Type            | Required | Description                                                                                           |
|----------------------|-----------------|----------|-------------------------------------------------------------------------------------------------------|
| `input`              | array           | Yes      | An array of image objects to embed. Each image object should include data in base64 format.           |
| `model`              | string          | No       | The ID of the model to use for Image Embedding. The model should be pre-configured for this service.  |
| `dimensions`         | integer (int32) | No       | The number of dimensions for the output embeddings.                                                   |
| `modelConfiguration` | object          | No       | Optional configuration parameters specific to the model.                                              |
...
```json
{
  ...
  "modelId": "domo.domo_ai.domo-embed-image-v1",
  "requestId": "123e4567-e89b-12d3-a456-426614174000"
}
```
