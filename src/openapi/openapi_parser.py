...
class OpenAPIParser:
            ...
            if isinstance(servers, list):
                for i, server in enumerate(servers):
                    if isinstance(server, dict) and not server.get("url"):
                        ...
                        Log.print_red(
                            f"Warning: Fixed empty server object at index {i}"
                        )
            ...
            for method in ["get", "post", "put", "delete", "patch", "options", "head"]:
                if method in path_item:
                    ...
                    endpoint = self._parse_endpoint(
                        path, method, operation, common_params
                    )
    ...
    def _parse_parameters(
        self, endpoint: EndpointData, parameters: List[Dict[str, Any]]
    ):
    ...
    def _extract_parameter_data(self, param: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not example:
            try:
                example = self.example_generator.generate_from_schema(
                    schema, param.get("name")
                )
            except Exception as e:
                Log.print_red(
                    f"Warning: Failed to generate example for {param.get('name')}: {e}"
                )
                example = None
    ...
    def _parse_request_body(
        ...
        existing_example = self.example_generator.extract_existing_example(
        ...
        for media_type, media_obj in content.items():
            ...
            if schema_description:
                ...
                endpoint.request_body_description = md(
                    schema_description, strip=["p"]
                ).strip()
            ...
            for prop_name, prop_schema in properties.items():
                # Store the original schema reference before any processing
                original_schema_ref = None
                if "$ref" in prop_schema:
                    original_schema_ref = prop_schema["$ref"]
                ...
                if not example:
                    try:
                        ...
                        Log.print_red(
                            f"Warning: Failed to generate example for {prop_name}: {e}"
                        )
                        example = None

                # Store the original schema reference for nested object processing
                schema_ref = original_schema_ref
            ...
            if not endpoint.request_body_example:
                try:
                    endpoint.request_body_example = (
                        self.example_generator.generate_from_schema(schema)
                    )
                except Exception as e:
                    Log.print_red(
                        f"Warning: Failed to generate request body example: {e}"
                    )
            ...
            for media_type, media_obj in content.items():
                ...
                if schema:
                    ...
                    if examples:
                        ...
                        try:
                            generated = self.example_generator.generate_from_schema(
                                schema
                            )
                            response_data["examples"].append(generated)
                        except Exception as e:
                            Log.print_red(
                                f"Warning: Failed to generate response example: {e}"
                            )
